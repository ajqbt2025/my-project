...
const ClientDetail = () => {
  ...
  return (
    <div className="flex justify-center bg-richblack-900 py-8">
      {/* PRINT AREA */}
      <div
        ref={printRef}
        className="w-[794px] min-h-[1123px] bg-white p-10 text-black font-inter"
      >
        ...
        <Section title="Occupation Details">
          ...
          {occupation?.occupation_type === "job" && (
            <>
              <Row
                label="Organisation"
                value={occupation.job_details?.org_name}
              />
              <Row
                label="Designation"
                value={occupation.job_details?.designation}
              />
              <Row label="Org ID" value={occupation.job_details?.org_id} />
              <Row
                label="Org Mobile"
                value={occupation.job_details?.org_mobile}
              />
              <Row
                label="Org Address"
                value={occupation.job_details?.org_address}
              />
            </>
          )}
          ...
          {occupation?.occupation_type === "business" && (
            <>
              <Row
                label="Shop Name"
                value={occupation.business_details?.shop_name}
              />
              <Row
                label="Category"
                value={occupation.business_details?.category}
              />
              <Row
                label="Business Year"
                value={occupation.business_details?.business_year}
              />
              <Row
                label="GST No"
                value={occupation.business_details?.gst_number}
              />
              <Row
                label="PAN No"
                value={occupation.business_details?.pan_number}
              />
            </>
          )}
        ...
        <Section title="Physical Condition">
          ...
          {physicalCondition?.condition === "abnormal" && (
            <>
              <Row
                label="Full Name"
                value={physicalCondition.abnormal_details?.full_name}
              />
              <Row
                label="UDID"
                value={physicalCondition.abnormal_details?.udid}
              />
              <Row
                label="Disability Type"
                value={physicalCondition.abnormal_details?.disability_type}
              />
              <Row
                label="DOB"
              ...
              <Row
                ...
                value={`${physicalCondition.abnormal_details?.disability_percent}%`}
              />
              <Row
                label="State ID"
                value={physicalCondition.abnormal_details?.state_id}
              />
              <Row
                label="UIDAI"
                value={physicalCondition.abnormal_details?.uidai}
              />
              <Row
                label="Issue Date"
              ...
              <Row
                label="Address"
                value={physicalCondition.abnormal_details?.address}
              />
            </>
          )}
        </Section>
        ...
        <Section title="Marital Status">
          ...
          <Row
            label="Marriage Date"
            value={
              maritalStatus.married_date
                ? maritalStatus.married_date.substring(0, 10)
                : "-"
            }
          />
          ...
          {maritalStatus.selected_status === "Single / Unmarried" && (
            <Row label="Remark" value="Not Married" />
          )}

          {maritalStatus.selected_status === "Married" && (
            <Row label="Remark" value="Currently Married" />
          )}

          {maritalStatus.selected_status === "Widowed" && (
            <Row label="Remark" value="Widowed" />
          )}

          {maritalStatus.selected_status === "Divorced" && (
            <Row label="Remark" value="Divorced" />
          )}

          {maritalStatus.selected_status === "Separated" && (
            <Row label="Remark" value="Separated" />
          )}

          {maritalStatus.selected_status === "Engaged / Betrothed" && (
            <Row label="Remark" value="Engaged / Betrothed" />
          )}

          {maritalStatus.selected_status === "Remarried" && (
            <Row label="Remark" value="Remarried" />
          )}

          {maritalStatus.selected_status === "Civil Partnership / Live-in" && (
            <Row label="Remark" value="Civil Partnership / Live-in" />
          )}

          {maritalStatus.selected_status === "Prefer not to say" && (
            <Row label="Remark" value="User chose not to say" />
          )}
        </Section>
...
const Row = ({ label, value }) => (
  <div className="flex items-center justify-center text-sm border-b border-dashed border-gray-400 py-1">
    {/* Label left side */}
    <span className="font-semibold w-[35%]">{label}</span>

    {/* Center line value */}
    <span className="w-[65%]  border-b border-gray-400">
